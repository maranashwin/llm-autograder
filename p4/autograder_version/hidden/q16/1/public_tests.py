...
def get_expected_json():
    ...
    expected_json = {'q1': 9.375,
                     ...
                     'q9': 5,
                     'q10': 'infinitely many',
                     'q11': {'Typhlosion', 'Infernape'},
                     'q12': {'Sylveon', 'Espeon'},
                     'q13': {'Terrakion', 'Volcanion'},
                     'q14': {'Miraidon', 'Koraidon'},
                     'q15': {'Greavard', 'Meowth'},
                     'q16': {'Stufful', 'Dragapult'}}
...
def rubric_check(rubric_point, check_past_errors=False):
    ...
    if msg != PASS:
        ...
        if msg == "hidden tests crashed before execution":
            ...
            comment = hidn.get_comment(rubric_point, check_past_errors)
...
def get_summary():
    ...
    try:
        if not os.path.exists(HIDDEN_FILE):
            ...
            return
        score = min(TOTAL_SCORE, hidn.get_score())
        display_msg = "Total Score: %d/%d" % (score, TOTAL_SCORE)
        if score != TOTAL_SCORE:
            display_msg += "\n" + hidn.get_deduction_string()
            display_msg += "Scroll up to the individual tests for more TEST DETAILS"
        print(display_msg)
    except:
